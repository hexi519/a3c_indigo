...
def construct_args(prog_args):
    ...
    args['ps_hosts'] = prog_args.ps_hosts   # "127.0.0.1:9000,127.0.0.1:8123"
    args['worker_hosts'] = prog_args.worker_hosts

    args['ps_list'] = prog_args.ps_hosts.split(',') # ["127.0.0.1:8123","127.0.0.1:9000"]
    args['worker_list'] = prog_args.worker_hosts.split(',')
    args['username'] = prog_args.username

    # change from ["127.0.0.1:8123","127.0.0.1:9000"] to ["hesy@127.0.0.1","hesy@127.0.0.1"]
...
def main():
    ...
    parser.add_argument(
        '--username', default='hesy',
        help='username used in ssh connection (default: hesy)')
    parser.add_argument(
        '--rlcc-dir', metavar='DIR', default='/home/hesy/pro/pan/a3c_indigo',
